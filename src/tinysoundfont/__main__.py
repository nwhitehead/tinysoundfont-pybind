...
def main():
    ...
    parser.add_argument('--info', action='store_true', help='Show information about SoundFont file')
    ...
    if args.test:
        ...
        for i in range(16):
            synth.program_select(i, sfid, args.bank[i % len(args.bank)], args.preset[i % len(args.preset)], is_drums=args.drum)
        for i in range(len(args.key)):
            synth.noteon(i, args.key[i % len(args.key)], args.velocity[i % len(args.velocity)])
        synth.start(buffer_size=args.buffer_size)
        time.sleep(3)
        synth.notes_off()
        time.sleep(1)
